...
public:
	c_hookmanager(void)
	{
		memset(this, 0, sizeof(c_hookmanager));
	}
	c_hookmanager(PDWORD* ppdwClassBase)
	{
...
extern oPaintTraverse paint_traverse_fn;
namespace hooks
{
	extern c_hookmanager* panel_vmt;
	extern c_hookmanager* clientmode_vmt;
	extern void __fastcall hk_paint_traverse(PVOID thisptr, int edx, unsigned int VGUIPanel, bool forceRepaint, bool allowForce);
	extern bool __stdcall hk_create_move(float flInputSampleTime, c_cmd* cmd);
	extern void run();
};