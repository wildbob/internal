...
// Netvar Manager
//------------------------------------------------------------
c_netvars* c_netvars::instance = nullptr;
...
void c_netvars::CreateDatabase()
{
	m_pDatabase = make_unique<NetvarDatabase>();
	auto pClient = g_instance.m_client( );

	if (pClient) {
		for (auto pClass = pClient->get_class();
			pClass;
			pClass = pClass->GetNextClass()) {
			if (pClass->GetTable()) {
				//Insert new entry on the database
				m_pDatabase->insert(
					pClass->GetTable()->m_pNetTableName,
					InternalLoadTable(pClass->GetTable(), 0));
				m_tableCount++;
			}
		}
	}
...
void c_netvars::Dump(std::ostream& output)
{
...
void c_netvars::Dump(const std::string& file)
{
...
//------------------------------------------------------------
unique_ptr<NetvarTable> c_netvars::InternalLoadTable(RecvTable* pRecvTable, uint32_t offset)
{
	...
	for (auto i = 0; i < pRecvTable->m_nProps; ++i) {
		...
		if (pProp->m_RecvType == (int)SendPropType::DPT_DataTable &&
...
void c_netvars::Dump(std::ostream& output, NetvarTable& table, int level)
{
	char line[512] = "";
	char fmt[512] = "";

	//yo dawg, i hear u like formatting strings so i am formatting ur formatting string
	sprintf_s(fmt, "%%-%ds: 0x%%08X\n", 50 - level * 4);
	...
	for (auto& prop : table.m_ChildProps) {
		...
		sprintf_s(line, fmt, prop.first.c_str(), prop.second + table.m_uOffset);
		output << line;
	...
	for (auto& child : table.m_ChildTables) {
		...
		sprintf_s(line, fmt, child.first.c_str(), child.second->m_uOffset);
		output << line;
		Dump(output, *child.second, level + 1);
	}
}

uint32_t c_netvars::GetOffset(const std::string& szTableName, const std::initializer_list<std::string>& props)
{
